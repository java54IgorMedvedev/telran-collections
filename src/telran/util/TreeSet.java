...
public class TreeSet<T> extends AbstractCollection<T> implements SortedSet<T> {
    ...
    public T floor(T key) {
        ...
        T floorValue = null;
        while (current != null) {
            int compRes = comp.compare(key, current.data);
            if (compRes == 0) {
    ...
    private void displayRootChildren(Node<T> node, int level) {
        if (node != null) {
            System.out.printf("%s%s%n", " ".repeat(level * spacesPerLevel), node.data);
    ...
    private Node<T> invertTree(Node<T> node) {
        ...
        Node<T> left = invertTree(node.left);
        Node<T> right = invertTree(node.right);
